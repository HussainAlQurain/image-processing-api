    ...
    async (
        ...
        try {
            ...
            if (
                ...
                res.send(
                    '<h1>Use query for filename to resize an image. use this example: http://localhost:3000/api/images?filename=fjord&width=200&height=200<br/>Or use only filename to display original: http://localhost:3000/api/images?filename=fjord</h1><br/><br/><h2 style="color:#eb4034">Available images are:<br/>encenadaport<br/>fjord<br/>icelandwaterfall<br/>palmtunnel<br/>santamonica</h2>'
                );
            } else {
                try {
